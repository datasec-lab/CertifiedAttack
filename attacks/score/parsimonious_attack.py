...
class ParsimoniousAttack(ScoreBlackBoxAttack):
    ...
    def local_search(self, image, noise, loss_fct, blocks):
        ...
        for _ in range(self.max_iters):
            ...
            for ibatch in range(num_batches):
                ...
                image_batch = torch.zeros([bend - bstart,self.img_size,self.img_size,self.in_channels]).float().to(self.device)
                noise_batch = torch.zeros([bend - bstart,self.img_size,self.img_size,self.in_channels]).float().to(self.device)
    ...
    def _perturb(self, xs_t, loss_fct):
        ...
        if self.is_new_batch:
            ...
            self.noise = -self.epsilon * torch.ones_like(xs_t).float().to(self.device)
        ...
        for i in range(num_batches):
            ...
            if self.query > self.max_loss_queries:
                return torch.clamp(xs_t + self.noise, 0, self.ub), t([num_queries])
            if success:
                return torch.clamp(xs_t + self.noise, 0, self.ub), t([num_queries])
