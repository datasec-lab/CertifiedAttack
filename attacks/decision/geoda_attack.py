...
class SubNoise(torch.nn.Module):
    ...
    def __init__(self, num_noises, x,device):
...
class GeoDAttack(DecisionBlackBoxAttack):
    ...
    def go_to_boundary(self, x_0, y_0, grad):
        ...
        while self.predict_label(perturbed) == y_0[0]:
            perturbed = x_0 + (num_calls*epsilon*grads[0].to(self.device))
            perturbed = torch.clamp(perturbed, 0, 1)
    ...
    def black_grad_batch(self, x_boundary, q_max, random_noises, y_0, sub_basis_torch):
        ...
        EstNoise = SubNoise(batch_size, sub_basis_torch,self.device)
        all_noises = []
        
        for j in range(num_batchs):
            if j == num_batchs-1:
                EstNoise_last = SubNoise(last_batch, sub_basis_torch,self.device)
    ...
    def GeoDA(self, x_0, y_0, x_b, iteration, q_opt, sub_basis_torch):
        ...
        for i in range(iteration):
            ...
            t1 = time.time()
            random_vec_o = torch.randn(q_opt[i], x_0.shape[1], x_0.shape[2], x_0.shape[3]).to(self.device)
